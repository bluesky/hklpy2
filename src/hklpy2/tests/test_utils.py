...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {"h": 0, "k": 0, "l": 0}, ["h", "k", "l"], does_not_raise(), id="dict-input"
        ),
        pytest.param(
            {"a": 0, "k": 0, "l": 0},
            ["h", "k", "l"],
        ...
        pytest.param(
            namedtuple("PseudoTuple", ["h", "k", "l"])(0, 0, 0),
            ["h", "k", "l"],
            does_not_raise(),
            id="namedtuple-input",
        ),
        pytest.param(
            np.array([0, 1, -1]), ["h", "k", "l"], does_not_raise(), id="ndarray-input"
        ),
        pytest.param(
            "123",
            ["h", "k", "l"],
        ...
        pytest.param(
            [0, 1, -1], ["aa", "bb", "cc"], does_not_raise(), id="custom-names"
        ),
        pytest.param(
            [1.1, 2.2, 3.3, 4, 5],
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            {"value": _RoundoffPos(8.98674, 12.00001), "digits": 4},
            does_not_raise(),
            id="namedtuple-returns-dict",
        ),
        pytest.param(
            {"value": {"a": 1.23456, "b": 0.0}, "digits": 4},
            does_not_raise(),
            id="mapping-returns-dict",
        ),
        pytest.param(
            {"value": [1.23456, 0.0], "digits": 4},
            does_not_raise(),
        ...
        pytest.param(
            {
                "value": _RoundoffNested(
                    outer=_RoundoffPos(1.23456, 2.34567), inner={"x": 3.45678}
                ),
                "digits": 4,
            },
            does_not_raise(),
        ...
        pytest.param(
            {"value": b"abc", "digits": 4},
            does_not_raise(),
            id="bytes-falls-through-to-repr",
        ),
        pytest.param(
            {"value": object(), "digits": 4},
            does_not_raise(),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            namedtuple("Position", ["a", "b", "c", "d"])(1, 2, 3, 4),
            AxesTuple,
            does_not_raise(),
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            namedtuple("Position", ["a", "b", "c"])(0, 0, 0),
            namedtuple("Position", ["a", "b", "c"])(1, 1, 1),
            1,
            1e-6,
            does_not_raise(),
            id="unit-distance",
        ...
        pytest.param(
            namedtuple("Position", ["a", "b", "c"])(0, 0, 0),
            namedtuple("NameIgnored", ["a", "b", "c"])(1, 0, 0),
            math.sqrt(1 / 3),
            1e-6,
            does_not_raise(),
            id="partial-offset",
        ...
        pytest.param(
            namedtuple("Position", ["x", "y", "z"])(0, 0, 0),
            namedtuple("Position", ["a", "b", "c"])(1, 1, 1),
            1,
            1e-6,
            pytest.raises(
        ...
        pytest.param(
            (),
            namedtuple("Ignored", ["a", "b", "c"])(1, 0, 0),
            0,
            1e-6,
            pytest.raises(AttributeError, match=re.escape("are not the same length.")),
            id="empty-vs-nonempty",
        ),
        pytest.param((), (), 0, 1e-6, does_not_raise(), id="both-empty"),
    ],
)
def test_distance_between_pos_tuples(pos1, pos2, dist, tol, context):
    with context:
        assert math.isclose(distance_between_pos_tuples(pos1, pos2), dist, abs_tol=tol)
    ...
    [
        ...
        pytest.param(
            ...
            [
                namedtuple("Position", ["a", "b", "c"])(1, -1, 1),
                namedtuple("Position", ["a", "b", "c"])(1, 1, 1),
                namedtuple("Position", ["a", "b", "c"])(3, 2, 1),
            ],
            pick_closest_solution,
            namedtuple("Position", ["a", "b", "c"])(1, -1, 1),  # first, closest
            does_not_raise(),
            id="pick-closest",
        ),
        pytest.param(
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            {"init_pos": 0, "physical_name": "linear", "kind": "hinted"},
            does_not_raise(),
            id="valid-specs",
        ),
        pytest.param(
        ...
        pytest.param(
            ...
            {"init_pos": 0, "physical_name": "guess"},
            pytest.raises(
                RuntimeError,
...
def test_virtual_axis(specs, context):
    GoniometerBase = diffractometer_class_factory(solver="hkl_soleil", geometry="E4CV")

    class Goniometer(GoniometerBase):
        tth = Component(VirtualPositionerBase, **specs)

        # Add the translation axis 'dy'.
        linear = Component(SoftPositioner, init_pos=0, limits=(-10, 200), kind="hinted")
    ...
    with context:
        ...
        assert math.isclose(
            gonio.tth.forward(gonio.tth.inverse(math.pi)), math.pi, abs_tol=0.01
        )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            EpicsMotor, {"prefix": "IOC:m1"}, does_not_raise(), id="EpicsMotor"
        ),
        pytest.param(MyPVPositioner, {}, does_not_raise(), id="PVPositioner"),
        pytest.param(SoftPositioner, {}, does_not_raise(), id="SoftPositioner"),
        pytest.param(
            Signal,
            {},
            pytest.raises(
...
def test_virtual_axis_physical(klass, specs, context):
    ...
    class Goniometer(GoniometerBase):
        tth = Component(
            VirtualPositionerBase, init_pos=0, physical_name="linear", kind="hinted"
        )
...
def test_virtual_axis_finish_setup_trigger():
    class SpecialCase(Device):
        physical = Component(
            SoftPositioner, init_pos=0, limits=(-10, 10), kind="hinted"
        )
        virtual = Component(
            VirtualPositionerBase,
            init_pos=0,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {"space": "reals", "canonical": ["aaa", "bbb", "ccc"]},
            does_not_raise(),
            id="Ok, default 'axes' & 'order'",
        ),
        pytest.param(
        ...
        pytest.param(
            ...
            pytest.raises(
                ValueError,
                match=re.escape("Duplicates in axes=['m1', 'm2', 'm3', 'm4', 'm1']"),
            ),
        ...
        pytest.param(
            {
                "space": "reals",
                "axes": {"m1": None, "m2": None, "m3": np.array([1, 2])},
                "canonical": ["aaa", "bbb", "ccc"],
            },
            pytest.raises(
                TypeError,
                match=re.escape("Incorrect type 'ndarray' for specs=array([1, 2])."),
            ),
...
def test_make_dynamic_instance_raises():
    ...
    with pytest.raises(TypeError, match=re.escape(f"{non_callable!r} is not callable")):
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            {"config": "e4cv_orient.yml", "print": False, "snapshot": True},
            does_not_raise(),
            id="benchmark returns dict when print=False (snapshot=True)",
        ),
        pytest.param(
            {"config": "e4cv_orient.yml", "print": True, "snapshot": True},
            does_not_raise(),
            id="benchmark returns None when print=True (snapshot=True)",
        ),
        pytest.param(
            {"config": "e4cv_orient.yml", "print": False, "snapshot": False},
            does_not_raise(),
            id="benchmark returns dict when print=False (snapshot=False)",
        ),
        pytest.param(
            {"config": "e4cv_orient.yml", "print": True, "snapshot": False},
            does_not_raise(),
...
def test_benchmark(capsys, parms, context):
    ...
    with context:
        ...
        result = benchmark(sim, n=10, print=parms["print"], snapshot=parms["snapshot"])
