...
``{{ obj.short_name }}({{ obj.args | shorten_type }}){% if obj.return_annotation is not none %} → {{ obj.return_annotation | shorten_type }}{% endif %}``
   ...
   {% endif %}
.. py:function:: {{ obj.short_name }}{% if obj.type_params %}[{{ obj.type_params }}]{% endif %}({{ obj.args | shorten_type }}){% if obj.return_annotation is not none %} -> {{ obj.return_annotation | shorten_type }}{% endif %}
